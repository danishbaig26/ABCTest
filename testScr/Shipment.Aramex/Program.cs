...
namespace Shipment.Aramex
{
    class Program
    {
        ...
        public string CreateShipments(out Int32 responseCode, out String responseMessage)
        {
            responseCode = 0;
    ...
    public class Utility
    {
        ...
        {
            string appSettingsValue = ConfigurationManager.AppSettings[appSettingsKey];
        ...
        /// <summary>
        /// Transform Transformed SoapResponse to ESB Class Object
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="TransformedResponse"></param>
        /// <returns></returns>
        public static T TransformToObject<T>(XDocument TransformedResponse)
        {
            if (TransformedResponse != null)
            {
                XmlSerializer s = new XmlSerializer(typeof(T));
                return (T)s.Deserialize(TransformedResponse.CreateReader());
            }
            else
            { return default(T); }
        }
        ...
        /// <returns></returns>
        public static T XmlSerialization<T>(XDocument xDocument)
        {
            try
            {
                StringReader stringReader = new StringReader(xDocument.ToString());

                XmlSerializer xmlSerializer = new XmlSerializer(typeof(T), new XmlRootAttribute(xDocument.Root.Name.LocalName));
                //XmlSerializer xmlSerializer = new XmlSerializer(typeof(T));

                var serializedResponse = (T)xmlSerializer.Deserialize(stringReader);
                return serializedResponse;
            }
            catch (Exception ex)
            {
                throw ex;
                //return default(T); //testing

            }
        }

        public static T XmlSerialization<T>(XElement xElement)
        {
            StringReader stringReader = new StringReader(xElement.ToString());
        ...
        /// <summary>
        /// Transform Transformed SoapResponse to DataSet
        /// </summary>
        /// <param name="TransformedResponse"></param>
        /// <returns></returns>
        public static DataSet TransformToDS(XDocument TransformedResponse)
        {
            DataSet ds = new DataSet();
            ds.ReadXml(TransformedResponse.CreateReader());
            return ds;
        }

        public static string TransformToObjectToXML<T>(T objEnv, XmlSerializerNamespaces ns)
        {

            System.Xml.Serialization.XmlSerializer x = new System.Xml.Serialization.XmlSerializer(objEnv.GetType());
            XmlDocument xmlDoc = new XmlDocument();
            using (MemoryStream xmlStream = new MemoryStream())
            {
                x.Serialize(xmlStream, objEnv, ns);
